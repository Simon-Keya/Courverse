...
export default function PublisherCoursesPage() {
  const courses = useQuery({
    queryKey: ["publisher", "courses", "courses"],
    queryFn: () => publisherApi.myCourses({ limit: 50 }),
    enabled: "courses" === "courses" || "courses" === "quizzes",
  });

  return (
    <div className="container-page py-8">
      <h1 className="font-heading text-2xl font-bold text-text">Publisher · Courses</h1>
      <p className="mt-1 text-sm text-text-secondary">
        Live data only. Dedicated courses APIs will expand this view.
      </p>
      {courses.isLoading && (
