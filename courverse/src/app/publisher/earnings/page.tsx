...
export default function PublisherEarningsPage() {
  const courses = useQuery({
    queryKey: ["publisher", "courses", "earnings"],
    queryFn: () => publisherApi.myCourses({ limit: 50 }),
    enabled: "earnings" === "courses" || "earnings" === "quizzes",
  });

  return (
    <div className="container-page py-8">
      <h1 className="font-heading text-2xl font-bold text-text">Publisher · Earnings</h1>
      <p className="mt-1 text-sm text-text-secondary">
        Live data only. Dedicated earnings APIs will expand this view.
      </p>
      {courses.isLoading && (
