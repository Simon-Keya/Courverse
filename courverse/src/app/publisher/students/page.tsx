...
export default function PublisherStudentsPage() {
  const courses = useQuery({
    queryKey: ["publisher", "courses", "students"],
    queryFn: () => publisherApi.myCourses({ limit: 50 }),
    enabled: "students" === "courses" || "students" === "quizzes",
  });

  return (
    <div className="container-page py-8">
      <h1 className="font-heading text-2xl font-bold text-text">Publisher · Students</h1>
      <p className="mt-1 text-sm text-text-secondary">
        Live data only. Dedicated students APIs will expand this view.
      </p>
      {courses.isLoading && (
