...
export default function PublisherQuizzesPage() {
  const courses = useQuery({
    queryKey: ["publisher", "courses", "quizzes"],
    queryFn: () => publisherApi.myCourses({ limit: 50 }),
    enabled: "quizzes" === "courses" || "quizzes" === "quizzes",
  });

  return (
    <div className="container-page py-8">
      <h1 className="font-heading text-2xl font-bold text-text">Publisher · Quizzes</h1>
      <p className="mt-1 text-sm text-text-secondary">
        Live data only. Dedicated quizzes APIs will expand this view.
      </p>
      {courses.isLoading && (
